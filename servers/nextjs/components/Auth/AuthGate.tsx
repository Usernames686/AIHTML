...
export default function AuthGate() {
  ...
  useEffect(() => {
    if (status.authenticated) {
      router.replace("/slidecraft");
    }
  }, [router, status.authenticated]);
  ...
  if (status.authenticated) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#F7F8FA] p-6">
        <div className="rounded-lg border border-[#E1E4EA] bg-white px-6 py-5 text-center shadow-sm">
          <img src="/logo-with-bg.png" alt="Presenton" className="mx-auto mb-4 h-10 w-10" />
          <p className="font-syne text-sm text-[#475467]">Opening SlideCraft...</p>
        </div>
      </main>
    );
  }
